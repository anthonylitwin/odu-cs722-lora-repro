...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Fine-tune RoBERTa on GLUE (MNLI/MRPC) with full / LoRA / BitFit."
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="Number of warmup steps for the LR schedule (0 = no warmup).",
    )
    parser.add_argument(
        "--max_seq_length",
        type=int,
...
def main():
    ...
    if args.max_train_samples:
        encoded_datasets["train"] = encoded_datasets["train"].select(
            range(args.max_train_samples)
        )
    if args.max_eval_samples:
        for split in encoded_datasets:
            if split != "train":
                encoded_datasets[split] = encoded_datasets[split].select(
    ...
    trainer = Trainer(
        ...
        else encoded_datasets["validation"],
        tokenizer=tokenizer,
